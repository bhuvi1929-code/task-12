...
export default function EmployeeTable({
  ...
  const handleExport = () => {
    exportEmployeesToCsv(employees, `employees-filtered-${new Date().toISOString().slice(0, 10)}.csv`);
  };
      ...
      <div className={styles.tableCard}>
        {employees.length === 0 ? (
          <div className={styles.tableEmptyState}>
            No employees match the current filters and search term. Try adjusting your criteria.
          </div>
        ) : (
          <>
                  ...
                  <tr>
                    {COLUMNS.map((col) => (
                      <th key={col.key} aria-sort={sortBy === col.key ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'}>
                        <button type="button" className={styles.sortButton} onClick={() => handleSort(col.key)}>
                          {col.label}
                          <ArrowUpDown size={12} />
                        </button>
                      </th>
                ...
                <tbody>
                  {pageItems.map((emp) => (
                    <tr key={emp.id} tabIndex={0}>
                      ...
                      <td>{emp.location}</td>
                      <td>
                        <span className={`${styles.statusPill} ${statusClass(emp.status)}`}>
                          {emp.status.replace('-', ' ')}
                        </span>
                      </td>
                      <td>
                        <span className={`${styles.riskPill} ${riskClass(emp.riskLevel)}`}>{emp.riskLevel}</span>
                      </td>
                      <td>{new Date(emp.hireDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</td>
            ...
            <div className={styles.tableFooter}>
              <span>
                ...
                &ndash;{Math.min(currentPage * PAGE_SIZE, employees.length)} of {employees.length}
              </span>
